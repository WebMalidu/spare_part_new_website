...
const loadProductCatalog = async (modelHasId, categoryItemId, stockId = "1", originId = "1") => {
     ...
     try {
          ...
          const brandCheckBoxContainer = document.getElementById('brandCheckBoxContainer');
          const brandCheckBoxNameContainer = document.getElementById('brandCheckBoxNameContainer');
...
document.getElementById('productStatusCheckbox1').addEventListener('change', () => {
     const categoryItemId = document.body.dataset.category_item_id;
     const radio = document.getElementById('productStatusCheckbox1');
     statusId = radio.value;
     radio.checked = true ? loadProductCatalog(modelHasId, categoryItemId, radio.value, originId) : loadProductCatalog(modelHasId, categoryItemId);

});


//get a radio out of stock
document.getElementById('productStatusCheckbox2').addEventListener('change', () => {
     const radio2 = document.getElementById('productStatusCheckbox2');
     statusId = radio2.value;
     const categoryItemId = document.body.dataset.category_item_id;
     radio2.checked = true ? loadProductCatalog(modelHasId, categoryItemId, radio2.value, originId) : loadProductCatalog(modelHasId, categoryItemId);
...
document.getElementById('originCheckbox1').addEventListener('change', () => {
     ...
     radio.checked = true ? loadProductCatalog(modelHasId, categoryItemId, statusId, radio.value) : loadProductCatalog(modelHasId, categoryItemId);
...
document.getElementById('originCheckbox2').addEventListener('change', () => {
     ...
     const categoryItemId = document.body.dataset.category_item_id;
     radio2.checked = true ? loadProductCatalog(modelHasId, categoryItemId, statusId, radio2.value) : loadProductCatalog(modelHasId, categoryItemId);
});
...
const loadCategory = async () => {
     ...
     try {
          const responseData = await fetch(SERVER_URL + "backend/api/categoriesLoad.php");
