...
if (!isset($_POST['category_id']) && !isset($_FILES['new_category_image']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
...
if (is_string($imagePath) && file_exists($imagePath)) {
     if (unlink($imagePath)) {
          ...
          if ($_FILES['new_category_image']['error'] === 0) {
               $fileExtension = strtolower(pathinfo($_FILES['new_category_image']['name'], PATHINFO_EXTENSION));
               ...
               if (in_array($fileExtension, $fileExtensions)) {
                    ...
                    if (move_uploaded_file($_FILES['new_category_image']['tmp_name'], $savePath . $newImageName)) {
