...
if (!isset($_POST['category_name']) && !isset($_FILES['category_item_image']) && !isset($_POST['category_id'])) {
     $responseObject->error = 'Access denied';
     response_sender::sendJson($responseObject);
}

// input data
$category_item_name = $_POST['category_name'];
...
$searchQuery = "SELECT * FROM `category_item` WHERE  `category`=? AND `category_category_id`=?";
...
if ($_FILES['category_item_image']['error'] === 0) {
     ...
     if (in_array($fileExtension, $allowImageExtension)) {
          ...
          if (move_uploaded_file($_FILES['category_item_image']['tmp_name'], $savePath . $newImageName)) {
               // data insert
               $insertCategory = "INSERT INTO `category_item` (`category_item_id`,`category`,`category_category_id`) VALUES (?,?,?)";
